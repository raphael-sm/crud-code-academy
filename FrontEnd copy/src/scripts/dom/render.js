...
export async function renderUsers(apiUrl) {
    ...
    if (users.length === 0) {
        usersSection.innerHTML = '<p class="text-muted">Nenhum Usuário Encontrado.</p>';
        return;
    }
        ...
        userDiv.innerHTML = `
            <div class="card user-card h-100" id="${escapeHtml(user.id)}">
                <div class="card-body">
                    ...
                    <p class="card-text mb-1"><strong>Idade:</strong> ${escapeHtml(user.age)}</p>
                    <p class="card-text"><strong>Email:</strong> ${escapeHtml(user.email)}</p>
                </div>
                <div class="card-footer d-flex gap-2">
                    <button class="btn btn-sm btn-outline-dark flex-fill" data-action="edit">Editar</button>
                    <button class="btn btn-sm btn-outline-danger flex-fill" data-action="delete">Deletar</button>
                </div>
            </div>
        `;
